...
import { getBarang, getTransaksiMasuk, getTransaksiKeluar, createTransaksiMasuk, createTransaksiKeluar } from '../lib/queries'
...
export function Inbound() {
  ...
  const bulanIni      = transaksi.filter((t: any) => {
    const d = new Date(t.tanggal); return d.getMonth() === month && d.getFullYear() === year
  })
  const totalBulanIni = bulanIni.reduce((a: number, t: any) => a + t.jumlah, 0)
  const terverifikasi = transaksi.length
  const menunggu      = 0
  const nilaiTotal    = transaksi.reduce((a: number, t: any) => a + Number(t.barang?.harga ?? 0) * t.jumlah, 0)
  ...
  const filtered = transaksi.filter((t: any) => {
    ...
    const matchPeriod = filterPeriid(t.tanggal, filterPeriod)
    return matchSearch && matchPeriod
  })

  function filterPeriid(tanggal: string, period: string): boolean {
  ...
  function exportCSV() {
    const rows = [
      'No. Transaksi,Tanggal,Barang,SKU,Jumlah,Keterangan,Oleh',
      ...transaksi.map((t: any) => {
        ...
        return `"${no}","${tgl}","${t.barang?.nama_barang ?? ''}","${t.barang?.sku ?? ''}",${t.jumlah},"${t.keterangan ?? ''}","${t.pengguna?.nama_lengkap ?? ''}"`
      })
    ]
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:16, marginBottom:20 }}>
        {[
          ...
          { label:'Nilai Total',     value: nilaiTotal >= 1_000_000 ? `${(nilaiTotal/1_000_000).toFixed(1)}M` : fmtNum(nilaiTotal),
            icon:<span style={{ fontWeight:700, fontSize:13 }}>Rp</span>, iconBg:'#EFF6FF', iconColor:'#2563EB' },
        ].map((s, i) => (
...
export function Outbound() {
  ...
  const totalBulanIni = bulanIni.reduce((a: number, t: any) => a + t.jumlah, 0)
  const selesai       = transaksi.length
  const dalamProses   = 0
  const nilaiTotal    = transaksi.reduce((a: number, t: any) => a + Number(t.barang?.harga ?? 0) * t.jumlah, 0)
    ...
    <>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:16, marginBottom:20 }}>
        {[
          ...
          { label:'Nilai Total',     value: nilaiTotal >= 1_000_000 ? `${(nilaiTotal/1_000_000).toFixed(1)}M` : fmtNum(nilaiTotal),
            icon:<span style={{ fontWeight:700, fontSize:13 }}>Rp</span>, iconBg:'#F5F3FF', iconColor:'#7C3AED' },
        ].map((s, i) => (
          <div key={i} style={{ background:'#fff', borderRadius:14, border:'1px solid #EAECF0', padding:'16px 20px', display:'flex', alignItems:'center', gap:16 }}>
            <div style={{ width:44, height:44, borderRadius:12, background:s.iconBg, color:s.iconColor, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>{s.icon}</div>
            <div>
...
export function InboundForm() {
  ...
  const mut = useMutation({
    mutationFn: () => createTransaksiMasuk({ data: {
  ...
  const stokSetelah = selectedBarang ? selectedBarang.kuantitas_stok + (Number(jumlah) || 0) : 0
    ...
    <div style={{ maxWidth:700, margin:'0 auto' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:24 }}>
        ...
        <div>
          ...
          <p style={{ fontSize:13, color:'#9CA3AF', marginTop:2 }}>Catat penerimaan stok masuk dari supplier</p>
        </div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 280px', gap:16, alignItems:'start' }}>
        ...
        <div className="wt-card" style={{ padding:'24px 28px' }}>
            ...
            <select style={{ ...fi, borderColor: errors.idBarang ? '#F87171' : '#E5E7EB', cursor:'pointer' }}
              ...
              {(barangList as any[]).map((b: any) => (
                <option key={b.id_barang} value={b.id_barang}>
                  [{b.sku}] {b.nama_barang} — Stok: {b.kuantitas_stok} {b.satuan}
                </option>
              ))}
            </select>
          ...
          <div style={fg}>
            <label style={fl}>Keterangan / Supplier <span style={{ color:'#9CA3AF', fontWeight:400 }}>(opsional)</span></label>
            <input style={fi} placeholder="Contoh: PT Maju Jaya, PO-2026-001"
              value={keterangan} onChange={e => setKeterangan(e.target.value)} />
          </div>

          <div style={{ display:'flex', justifyContent:'flex-end', gap:8, marginTop:8 }}>
            <button className="btn btn-secondary btn-sm" onClick={() => navigate({ to:'/inbound' })} disabled={mut.isPending}>
              Batal
            </button>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          <div className="wt-card" style={{ padding:'20px 22px' }}>
            ...
            {!selectedBarang ? (
              <div style={{ fontSize:13, color:'#9CA3AF', textAlign:'center', padding:'16px 0' }}>Pilih barang untuk melihat ringkasan</div>
            ) : (
              <dl style={{ display:'flex', flexDirection:'column', gap:10, margin:0 }}>
                {[
                  ...
                  { k:'Stok Saat Ini',   v: `${fmtNum(selectedBarang.kuantitas_stok)} ${selectedBarang.satuan}` },
          ...
          <div style={{ background:'#EBF5EE', borderRadius:12, padding:'12px 16px', fontSize:12.5, color:'#2E7D52', lineHeight:1.6 }}>
            ✓ Stok akan bertambah otomatis setelah disimpan
          </div>
        </div>
      </div>
    </div>
...
export function OutboundForm() {
  ...
  const stokSetelah    = selectedBarang ? selectedBarang.kuantitas_stok - jumlahNum : 0
  const stokCukup      = selectedBarang ? selectedBarang.kuantitas_stok >= jumlahNum : true
  const nilaiTotal     = selectedBarang ? Number(selectedBarang.harga) * jumlahNum : 0

  const mut = useMutation({
    mutationFn: () => createTransaksiKeluar({ data: {
      id_barang:   Number(idBarang),
      id_pengguna: getSessionUserId(),
      jumlah:      jumlahNum,
      keterangan:  keterangan || undefined,
  ...
  function validate() {
    ...
    else if (selectedBarang && jumlahNum > selectedBarang.kuantitas_stok)
      e.jumlah = `Stok tidak cukup. Tersedia: ${selectedBarang.kuantitas_stok} ${selectedBarang.satuan}.`
    if (!tanggal)                 e.tanggal  = 'Tanggal wajib diisi.'
    setErrors(e)
    return Object.keys(e).length === 0
  }
    ...
    <div style={{ maxWidth:700, margin:'0 auto' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:24 }}>
        ...
        <div>
          ...
          <p style={{ fontSize:13, color:'#9CA3AF', marginTop:2 }}>Catat pengeluaran atau distribusi barang</p>
        </div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 280px', gap:16, alignItems:'start' }}>
        ...
        <div className="wt-card" style={{ padding:'24px 28px' }}>
          ...
          <div style={fg}>
            <label style={fl}>Pilih Barang <span style={{ color:'#EF4444' }}>*</span></label>
            <select style={{ ...fi, borderColor: errors.idBarang ? '#F87171' : '#E5E7EB', cursor:'pointer' }}
              value={idBarang} onChange={e => { setIdBarang(e.target.value); setJumlah(''); setErrors(p => ({ ...p, idBarang:'' })) }}>
              <option value="">— Pilih barang —</option>
              {(barangList as any[]).map((b: any) => (
                <option key={b.id_barang} value={b.id_barang} disabled={b.kuantitas_stok === 0}>
                  [{b.sku}] {b.nama_barang} — Stok: {b.kuantitas_stok} {b.satuan}{b.kuantitas_stok === 0 ? ' (HABIS)' : ''}
                </option>
              ))}
            </select>
            {errors.idBarang && <span style={errS}>{errors.idBarang}</span>}
          ...
          <div style={fg}>
            ...
            <input style={{ ...fi, borderColor: errors.jumlah ? '#F87171' : '#E5E7EB' }}
              type="number" min="1" max={selectedBarang?.kuantitas_stok ?? undefined}
              placeholder="Contoh: 10"
              value={jumlah} onChange={e => { setJumlah(e.target.value); setErrors(p => ({ ...p, jumlah:'' })) }} />
            {errors.jumlah && <span style={errS}>{errors.jumlah}</span>}
            {selectedBarang && (
              <span style={{ fontSize:12, color: stokCukup ? '#9CA3AF' : '#DC2626', marginTop:4, display:'block' }}>
                Stok tersedia: {selectedBarang.kuantitas_stok} {selectedBarang.satuan}
              </span>
            )}
          </div>
          ...
          <div style={fg}>
            <label style={fl}>Keterangan / Tujuan <span style={{ color:'#9CA3AF', fontWeight:400 }}>(opsional)</span></label>
            <input style={fi} placeholder="Contoh: Toko Cabang Bandung, ORD-2026-001"
              value={keterangan} onChange={e => setKeterangan(e.target.value)} />
          </div>

          <div style={{ display:'flex', justifyContent:'flex-end', gap:8, marginTop:8 }}>
            <button className="btn btn-secondary btn-sm" onClick={() => navigate({ to:'/outbound' })} disabled={mut.isPending}>
              Batal
            </button>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          ...
          {selectedBarang && !stokCukup && jumlah && (
          ...
          {stokCukup && (
            <div style={{ background:'#FEF9C3', borderRadius:12, padding:'12px 16px', fontSize:12.5, color:'#CA8A04', lineHeight:1.6 }}>
              ✓ Stok akan berkurang otomatis setelah disimpan
            </div>
          )}
        </div>
      </div>
